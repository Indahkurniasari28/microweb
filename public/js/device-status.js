...
function initDeviceDetailPage() {
  ...
  const DEVICES = {
    ...
    spectrometer: {
      name: 'Avantes UV Spektrofotometer',
  ...
  let stateDesc  = 'Perangkat tidak terdeteksi. Periksa koneksi jaringan.';
  ...
  if (deviceId === 'raspberry') {
    if (isConnected) {
      ...
      stateDesc  = isMeasuring
        ? 'Sistem aktif menjalankan sesi pengukuran. Data diterima secara real-time.'
        : 'Instrumen online tetapi tidak ada sesi aktif. Menunggu inisiasi protokol.';
    } else {
      stateDesc = 'Raspberry Pi tidak terdeteksi. Periksa koneksi server dan jaringan.';
    }
    ...
    if (isConnected) {
      ...
      stateDesc  = hasData
        ? 'Spektrofotometer aktif mengambil data absorbansi secara real-time via USB.'
        : 'Spektrofotometer terhubung via USB. Kalibrasikan dark current sebelum memulai.';
    } else {
      stateDesc = 'Avantes Spektrofotometer tidak terdeteksi. Periksa koneksi USB ke Raspberry Pi.';
    }
    ...
    if (esp32Online) {
      ...
      stateDesc  = 'ESP32 mengirim data sensor via MQTT. Data distance & rain diterima.';
    } else if (isConnected) {
      state      = 'IDLE';
      stateColor = 'text-secondary';
      dotColor   = 'bg-secondary';
      stateDesc  = 'Server terhubung namun belum ada data MQTT dari ESP32. Periksa konfigurasi WiFi ESP32.';
    } else {
      stateDesc = 'ESP32 tidak terdeteksi. Pastikan ESP32 terhubung ke WiFi dan MQTT broker aktif.';
    }
  }
  ...
  if (uptimeEl)   uptimeEl.textContent   = 'Last check: ' + new Date().toLocaleTimeString('id-ID');
  ...
  if (deviceId === 'spectrometer') {
    ...
    if (tlStart)    tlStart.textContent = new Date(Date.now() - 3600000).toLocaleTimeString('id-ID');
    if (tlConn)     tlConn.textContent  = new Date(Date.now() - 3000000).toLocaleTimeString('id-ID');
    if (latest && tlReacLbl) {
      tlReacLbl.textContent = 'Pengukuran Berlangsung';
    ...
    grid.innerHTML = `
      <div class="col-span-2 glass-panel rounded-xl p-lg flex flex-col gap-sm">
        <div class="flex items-center gap-xs">
          ...
          <span class="font-label-caps text-[10px] text-on-surface-variant uppercase tracking-widest">Degradasi</span>
      ...
      <div class="glass-panel p-md rounded-xl flex flex-col gap-xs">
        <div class="flex items-center gap-xs">
          ...
          <span class="font-label-caps text-[10px] text-on-surface-variant uppercase tracking-widest">Konsentrasi</span>
        </div>
...
function _renderDeviceHealth(deviceId, isConnected, state) {
  ...
  if (deviceId === 'raspberry') {
    ...
    items = [
      { icon: 'memory',    label: 'RPi 5 Host',    sub: 'Driver aktif via USB', statusKey: isConnected ? 'connected' : 'offline', statusLabel: isConnected ? 'CONNECTED' : 'OFFLINE', color: 'text-tertiary' },
      ...
      { icon: 'biotech',   label: 'Spektrofotometer', sub: isConnected ? 'Dark current calibrated' : 'No signal', statusKey: isConnected ? (state==='CAPTURING'?'active':'standby') : 'offline', statusLabel: isConnected ? state : 'OFFLINE', color: 'text-primary' },
    ...
    items = [
      { icon: 'sensors',      label: 'Sensor Ultrasonik HC-SR04', sub: esp32On ? `${esp32RealData.distance?.toFixed(1)||'-'} cm terakhir` : 'Menunggu data', statusKey: esp32On ? 'active' : 'offline', statusLabel: esp32On ? 'ACTIVE' : 'OFFLINE', color: 'text-secondary' },
      { icon: 'water_drop',   label: 'Sensor Hujan',    sub: esp32On ? (esp32RealData.rainStatus||'-') : '-', statusKey: esp32On ? 'connected' : 'offline', statusLabel: esp32On ? 'ACTIVE' : 'OFFLINE', color: 'text-primary' },
      ...
      { icon: 'hub',          label: 'MQTT Broker',     sub: isConnected ? 'Broker aktif via server' : 'Offline', statusKey: isConnected ? 'connected' : 'offline', statusLabel: isConnected ? 'CONNECTED' : 'OFFLINE', color: 'text-secondary' },
    ];
  }
...
function _renderDeviceLog(deviceId, isConnected, state) {
  ...
  const now  = new Date();
  const fmt  = d => d.toLocaleTimeString('id-ID');
  const logs = [];

  if (deviceId === 'raspberry') {
    logs.push(
      { ok: true,  time: fmt(new Date(now - 5000)),  msg: 'System heartbeat OK — server terhubung' },
      ...
      { ok: false, time: fmt(new Date(now - 60000)), msg: 'WARN: CPU temp spike 48°C — kembali normal' },
    );
  } else if (deviceId === 'spectrometer') {
    const latest = window.measurementHistory?.[0];
    if (latest) {
      logs.push({ ok: true, time: fmt(now), msg: `Degradasi: ${(latest.degradation||0).toFixed(1)}% — Konsentrasi: ${(latest.concentration||0).toFixed(2)} ppm` });
    }
    logs.push(
...
function _renderDeviceEvents(deviceId, state) {
  ...
  if (deviceId === 'raspberry') {
    ...
    events = [
      ...
      { time: '18 May 2026 08:55', code: 'INFO-011', desc: 'Dark current calibration berhasil', type: 'info', status: 'OK' },
      { time: '17 May 2026 14:20', code: 'INFO-001', desc: 'Sesi pengukuran selesai — degradasi 87.3%', type: 'info', status: 'OK' },
    ];
  } else if (deviceId === 'esp32') {
    events = [
      { time: '18 May 2026 14:50', code: 'ERR-401', desc: 'Packet loss — sensor heartbeat tidak diterima', type: 'error',   status: 'Auto-recovered' },
      { time: '18 May 2026 13:10', code: 'WRN-202', desc: 'WiFi RSSI rendah: -78 dBm', type: 'warning', status: 'Resolved' },
      { time: '17 May 2026 08:00', code: 'INFO-001', desc: 'ESP32 boot sequence selesai', type: 'info', status: 'OK' },
    ];
  }

  if (events.length === 0) {
    tbody.innerHTML = `<tr><td colspan="4" class="px-lg py-xl text-center text-on-surface-variant font-body-md">Tidak ada event tercatat</td></tr>`;
    return;
  }
